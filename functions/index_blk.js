$(document).ready(function () {
    ...
    $('#submit').click(async function () {
        ...
        try {
            ...
            await new Promise(async (resolve, reject) => {
                $.get(`https://api.godsunchained.com/v0/properties?user_id=${playerid}`, async (d) => {
                    try {
                        ...
                        $.get(`https://api.godsunchained.com/v0/match?game_mode=204&end=${currentTimestamp}-&player_won=${playerid}`, async (data) => {
                            try {
                                ...
                                $.get(`https://api.godsunchained.com/v0/match?game_mode=204&end=${currentTimestamp}-&player_won=${playerid}&page=${page}`, async (data) => {
                                    try {
                                        const obj = JSON.parse(data);
                                        if (obj.records && Array.isArray(obj.records)) {
                                            wonRecords.push(...obj.records.filter(x => x.game_mode === 204));
                                            console.log("API call 3 complete");
                                        } else {
                                            console.error("Error: No records found in won matches API response");
                                            displayErrorMessage("No records found for your won matches.");
                                            $('#loading').hide();
                                            reject("No records found for won matches");
                                        ...
                                        if (page > 1) {
                                            $.get(`https://api.godsunchained.com/v0/match?game_mode=204&end=${currentTimestamp}-&player_won=${playerid}&page=${page - 1}`, async (data) => {
                                                try {
                                                    const obj = JSON.parse(data);
                                                    if (obj.records && Array.isArray(obj.records)) {
                                                        wonRecords.push(...obj.records.filter(x => x.game_mode === 204));
                                                        ...
                                                        console.error("Error: No records found in won matches second last page.");
                                                    }
                                    ...
                                    displayErrorMessage("Error retrieving won matches. Please try again.");
                                    $('#loading').hide();
                                    reject("Error 2");
                                });
                            } catch (error) {
            ...
            await new Promise(async (resolve, reject) => {
                $.get(`https://api.godsunchained.com/v0/match?game_mode=204&end=${currentTimestamp}-&player_lost=${playerid}`, async (data) => {
                    try {
                        const obj = JSON.parse(data);
                        ...
                        $.get(`https://api.godsunchained.com/v0/match?game_mode=204&end=${currentTimestamp}-&player_lost=${playerid}&page=${page2}`, async (data) => {
                            try {
                                const obj = JSON.parse(data);
                                if (obj.records && Array.isArray(obj.records)) {
                                    lostRecords.push(...obj.records.filter(x => x.game_mode === 204));
                                    console.log("API call 6 complete");
                                } else {
                                    console.error("Error: No records found in lost matches API response.");
                                    displayErrorMessage("No records found for your lost matches.");
                                    $('#loading').hide();
                                    reject("No records found for lost matches");
                                    return; // Stop further execution
                                }
                                ...
                                if (page2 > 1) {
                                    $.get(`https://api.godsunchained.com/v0/match?game_mode=204&end=${currentTimestamp}-&player_lost=${playerid}&page=${page2 - 1}`, async (data) => {
                                        try {
                                            const obj = JSON.parse(data);
                                            if (obj.records && Array.isArray(obj.records)) {
                                                lostRecords.push(...obj.records.filter(x => x.game_mode === 204));
                                                ...
                                                console.error("Error: No records found in lost matches second last page.");
                                            }
                                            ...
                                            console.error("Error retrieving lost matches from second last page:", error);
                                            reject("Error retrieving lost matches from second last page");
                                        }
                                    }).fail(function () {
                                        displayErrorMessage("Error retrieving lost matches from second last page. Please try again.");
                                        $('#loading').hide();
                                        reject("Error retrieving lost matches from second last page");
                                    });
                                } else {
                            ...
                            displayErrorMessage("Error retrieving lost matches. Please try again.");
                            $('#loading').hide();
                            reject("Error 2");
                        });
                    } catch (error) {
            ...
            // This part will only run if both won and lost records were found
            ...
            if (output.children().length === 0) {
                ...
                resultDiv.append(`<p>${wonRecords.length} W - ${lostRecords.length} L</p>`);
                ...
                for (let x of filtered) {
                    ...
                    const opponentid = [player_won, player_lost].find((id) => id !== parseInt(playerid));
                    const opponentUsername = await getOpponentUsername(opponentid);
                            ...
                            <div id="${id}" class="versus-container">
                                ...
                                <div class="player-container">
                                    ...
                                    <div class="opponent-container">
                                        <div class="container">
                                            <div><h2><a style="text-decoration: underline;" target=_blank href=https://gudecks.com/meta/player-stats?userId=${opponentPlayerid}>${opponentUsername}</a></h2></div>
                                            <div class="god-result" style="text-transform: uppercase;">
